...
class AdminPortfolioIntegrationTests(unittest.TestCase):
    ...
    def test_admin_serializer_preserves_service_economics_without_recalculation(self):
        ...
        projection = StrategyPerformanceProjection(
            generated_at=now,
            quote_error="Alpaca quote delayed",
            strategy=source,
            ...
            runtime=PerformanceRuntime(
                last_successful_pipeline_at=now,
                last_successful_pipeline_run_id="run-serializer",
                last_pipeline_admitted=1,
                last_pipeline_stages=(
                    {"stage": "signal", "diagnostics": [{"accepted": 1}]},
                ),
                ...
                last_pipeline_data_quality={
                    ...
                    "quote_count": 19,
                    "basic_count": 10,
                    "history_ready_count": 8,
                    "strategy_filtered_count": 4,
                    "absolute_momentum_count": 2,
                    "selected_count": 1,
                    "source_mode": "primary_ready",
                },
            ),
            nav_history=(
                PerformanceNavPoint(
            ...
            positions=(
                PerformancePosition(
                    ...
                    name="Apple",
                    symbol="AAPL",
                    first_entry_price=120.0,
                    first_entry_at=now.replace(day=1),
                    current_price=118.0,
                    day_change_pct=-1.25,
                    return_pct=1.67,
                    unrealized_pnl=16.0,
            ...
            orders=(
                PerformanceOrder(
                    ...
                    status="PARTIAL",
                    reason="ENTRY_SIGNAL",
                    created_at=now.replace(hour=11, minute=55),
                    updated_at=now,
                    filled_notional=236.0,
                    ...
                    valid_session_date="2026-08-02",
                    replacement_candidate={"symbol": "MSFT"},
                ),
            ),
            closed_trades=(
                PerformanceClosedTrade(
            ...
            events=(
                PerformanceEventView(
                    ...
                    type="MARGIN_CALL",
                    occurred_at=now,
                    message="margin threshold reached",
                    strategy_revision=2,
                    key="risk-event-1",
                    data={"margin_rate_pct": 24.0, "action": "COVER_ONLY"},
                ),
            ),
        ...
        self.assertEqual(payload["orders"][0]["key"], "order-key-1")
        self.assertEqual(payload["orders"][0]["signal_price"], 120.0)
        self.assertEqual(payload["orders"][0]["replacement_candidate"], {"symbol": "MSFT"})
        self.assertEqual(
